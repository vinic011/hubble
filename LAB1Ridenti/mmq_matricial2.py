...
X = np.column_stack((ones_column, r))  # agrega vetor coluna 1 ao vetor coluna tempo para montar a matriz projeto
...
v_fit = np.matmul(X, b)  # valores ajustados segundo a solução
...
# Variância constante não afeta a estimativa dos parâmetros

# Calcula a matriz de covariância dos parâmetros
M_cov = var * np.linalg.inv(np.matmul(X.T, X))

# Calcula constante de Hubble
H0 = b[1]
s_H0 = np.sqrt(M_cov[1, 1])
